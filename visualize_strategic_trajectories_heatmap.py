...
- strategic_trajectories/*.csv - 轨迹数据
- strategic_paths/*.csv - 原始路径数据

输出文件:
- strategic_heatmap_visualization.png - 热力图可视化结果
"""
...
def load_data():
    ...
    if os.path.exists(trajectories_dir):
        ...
        print(f"已加载 {len(trajectories)} 条战略轨迹")
    else:
...
def main():
    ...
    # 添加标题和轴标签
    plt.title('战略轨迹热力图分析', fontsize=15)
    plt.xlabel('列 (东-西方向)')
    plt.ylabel('行 (南-北方向)')
    
    # 保存图像
    output_path = os.path.join(OUTPUT_DIR, "strategic_heatmap_visualization.png")
